...
public class MapReduceServletTest extends TestCase {
  ...
  private HttpServletRequest createMockStartJobRequest(Configuration conf) {
    HttpServletRequest request = createMockRequest(
        MapReduceServlet.COMMAND_PATH + "/" + CommandHandler.START_JOB_PATH, false, true);
  ...
  private HttpServletRequest createMockControllerRequest(int sliceNumber, JobID jobId) {
    HttpServletRequest request = createMockRequest(ControllerWorker.CONTROLLER_PATH, true, false);
  ...
  public void testControllerCSRF() {
    ...
    HttpServletRequest request = createMockRequest(ControllerWorker.CONTROLLER_PATH, false, true);
    expect(request.getParameter(AppEngineJobContext.SLICE_NUMBER_PARAMETER_NAME))
      .andReturn("" + 0)
      .anyTimes();
    expect(request.getParameter(AppEngineJobContext.JOB_ID_PARAMETER_NAME))
  ...
  public void testGetJobDetailCSRF() {
    ...
    HttpServletRequest request = createMockRequest(
        MapReduceServlet.COMMAND_PATH + "/" + CommandHandler.GET_JOB_DETAIL_PATH, true, false);
    expect(request.getMethod())
      .andReturn("POST")
      .anyTimes();
    expect(request.getParameter(AppEngineJobContext.JOB_ID_PARAMETER_NAME))
  ...
  public void testGetBase() {
    ...
    assertEquals("/mapreduce/", UrlUtil.getBase(request));
    verify(request);
  }
  ...
  public void testScheduleShards() {
    ...
    MapperWorker.scheduleShards(request, context, format, splits);
  ...
  public void testRefillQuotas_noneActive() {
    ...
    // catch this.
    servlet.controllerWorker.refillQuotas(null, null, activeShardStates);
  }
  ...
  public void testRefillQuotas() {
    ...
    ControllerWorker controller = new ControllerWorker(clock);
  ...
  public void testAggregateState() {
    ...
    servlet.controllerWorker.aggregateState(mrState, Arrays.asList(shardState1, shardState2));
  ...
  public void testHandleController_withContinue() throws EntityNotFoundException {
    ...
    TaskStateInfo firstTask = defaultQueue.getTaskInfo().get(0);
    assertEquals("/mapreduce/" + ControllerWorker.CONTROLLER_PATH, firstTask.getUrl());
    assertTrue(firstTask.getBody(), 
  ...
  public void testHandleController_withCleanup() {
    ...
    servlet.controllerWorker.handleController(request, null);
...
//  }
  ...
  public void testMapperWorker_withoutQuota() throws IOException {
    ...
    state.persist();
    servlet.mapperWorker.handleMapperWorker(request, null);
  ...
  public void testProcessMapper_noQuotaDoesntSkip() throws Exception {
    ...
    servlet.mapperWorker.processMapper(mapper, null, consumer, System.currentTimeMillis() + 10000);
  ...
  public void testHandleListConfigs() throws Exception {
    ...
    JSONObject retValue = servlet.commandHandler.handleListConfigs(new MapReduceXml(testStream));
    assertJsonEquals(
          "{'configs':["
        + "  {'mapper_params':"
        + "    {'Baz':"
  ...
  public void testHandleListJobs() throws Exception {
    ...
    JSONObject retObject = servlet.commandHandler.handleListJobs(null, 2);
    assertJsonEquals(
          "{'jobs':["
        + "  {"
    ...
    retObject = servlet.commandHandler.handleListJobs(retObject.getString("cursor"), 50);
    assertJsonEquals(
  ...
  public void testGetJobDetail_empty() throws Exception {
    ...
    state.persist();
    JSONObject result = servlet.commandHandler.handleGetJobDetail(state.getJobID());
    assertJsonEquals(
  ...
  public void testCleanupJob() throws Exception {
    ...
    state.setConfigurationXML(
        ConfigurationXmlUtil.convertConfigurationToXml(new Configuration(false)));
    state.persist();
    JSONObject response = servlet.commandHandler.handleCleanupJob(state.getJobID());
    try {
  ...
  public void testCommandError() throws Exception {
    HttpServletRequest request = createMockRequest(
        MapReduceServlet.COMMAND_PATH + "/" + CommandHandler.GET_JOB_DETAIL_PATH, false, true);
    expect(request.getMethod())
      .andReturn("GET")
      .anyTimes();
    HttpServletResponse response = createMock(HttpServletResponse.class);
